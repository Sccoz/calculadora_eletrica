let quantidade = parseInt(
  prompt("Digite quantos eletrodomesticos voce quer ver o preco: "),
);
let contador = 0;
...
while (quantidade > contador) {
  ...
  let consumo = parseFloat(
    prompt("Digite o consumo do eletro domestico em (W ou KW) : "),
  );
  let horas = parseFloat(
    prompt(`Digite mais ou menos quantas horas ele fica ligado por dia : `),
  );
  let dias = parseInt(
    prompt("Digite quantos dias em media ele fica ligado no mes : "),
  );
  switch (quilo_whats) {
    case 1:
